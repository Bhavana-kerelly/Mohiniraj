      ...
      <section className="relative px-6 sm:px-8 lg:px-12 max-w-7xl mx-auto py-16">
        <div className="space-y-6 max-w-4xl">
          ...
          <h1 className="text-5xl sm:text-7xl lg:text-8xl font-editorial font-light tracking-tight text-brand-charcoal leading-none">
            TRANSFORMATION <br />
            <span className="text-brand-champagne italic">EXPERIENCES.</span>
          </h1>
          <p className="text-lg sm:text-2xl text-brand-brown font-light leading-relaxed max-w-2xl">
            Choose the depth of engagement designed to align with your personal evolution.
          </p>
        </div>
      </section>

      {/* Detailed Modules Showcase */}
      <section className="py-16 px-6 sm:px-8 lg:px-12 max-w-7xl mx-auto space-y-16 border-t border-brand-charcoal/10">
        {DETAILED_EXPERIENCES.map((exp) => (
          <div
            key={exp.title}
            className="glass-panel-light p-8 sm:p-12 rounded-3xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative overflow-hidden group hover:border-brand-champagne/40 transition-all duration-500 shadow-xl"
          >
            <div className="lg:col-span-7 space-y-6">
              <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-brand-champagne">
                <span className="px-3 py-1 rounded-full border border-brand-champagne/30 bg-white/60 uppercase tracking-widest font-semibold">
                  {exp.type}
                </span>
              ...
              <h2 className="text-3xl sm:text-5xl font-editorial font-normal text-brand-charcoal group-hover:text-brand-champagne transition-colors">
                {exp.title}
              </h2>
              ...
              <div className="space-y-2 pt-2 border-t border-brand-charcoal/10">
                {exp.features.map((feat, fIdx) => (
                  <div key={fIdx} className="flex items-center gap-3 text-xs sm:text-sm text-brand-charcoal/90 font-light">
            ...
            <div className="lg:col-span-5 relative h-[320px] sm:h-[420px] rounded-2xl overflow-hidden shadow-xl border border-white/60">
              <Image
                src={exp.image}
                alt={exp.title}
                fill
